...
int fmt_mic_e(const u_char *t,          // tocall
    ...
    if (strlen((char*)t) != 6)          // AX25 dest addr must be 6 bytes long.
        return 0;
    ...
    for (x=0;x<6;x++) {
        ...
        if ((c > '9') && (c < 'A'))     // symbols :;<>?@ are invalid
            trash |= 2;

        if ((x >= 3) && (c >= 'A') && (c <= 'K'))   // A-K not allowed in last 3 bytes
            trash |= 4;
    }

    for (x=1;x<7;x++) {
        if ((i[x] < 0x1c) || (i[x] > 0x7f)) // Range check the longitude and speed/course
            trash |= 8;
    }

    //printf("Trash=%d %s %s \n",trash, t, i);   //Debug

    if (trash)                          // exit without converting packet if it's malformed
        return 0;
    ...
    *l1 = *l2 = 0;
    ...
    switch (i[0]) {                     // possible valid MIC-E flags
        case 0x60:                      // Current GPS data (not used in Kenwood TM-D700)
            ...
            if (l < 9)                  // reject packet if less than 9 bytes
                return 0;
        ...
        case 0x1c:                      // Rev 0 beta units current GPS data
            ...
            if (l < 8)                  // reject packet if less than 8 bytes
                return 0;
        ...
        default:                        // Don't attempt to process a packet that's not a Mic-E type
            return 0;
    }

    if ((i[9] == ']') || (i[9] == '>')) // Kenwood with messaging detected
        adti = '=';

    if (l >= 7 ) {                      // <-- old beta units could have only 7 bytes,
                                        // new units should have 9 or more bytes
        ...
        if (cse > 360)                  // Reject invalid course value
            return 0;

        if (spd > 999)                  // reject invalid speed value
            return 0;
        ...
        // rev 1 bug workaround:  sends null symbol/table bytes
        symbol = i[7];
        if (!isprint(symbol))           // Reject packet if invalid symbol code
            return 0;
        ...
        if (l >= 10) {
            ...
            switch (i[buf2_n]) {
                ...
                case 0x1d:          // five-channel telemetry (This works only if the input device
