        ...
        if (state == 2) {
            ...
            */
            ...
            if (pcp->user.length() != 0) {
                ...
                //memset(logonBuf, 0, 256);
                //ostrstream logon(logonBuf, 255);   //Build logon string
                ostringstream logon;
                logon << "user "
                    << pcp->user
                    << " pass "
                ...
                //rc = send(clientSocket, logonBuf, strlen(logonBuf), 0);
...
int SendFiletoClient(int session, char *szName)
{
    ...
    if (!file) {
        ...
        //pthread_mutex_unlock(pmtxSendFile);                //Added in 215
