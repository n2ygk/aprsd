...
cpQueue sendQueue(640, true);           // Internet transmit queue
cpQueue tncQueue(64, true);             // TNC RF transmit queue
cpQueue charQueue(64, false);           // A queue of single characters
cpQueue conQueue(64, true);             // data going to the console from various threads
...
RecursiveMutex mtxSendFile;
RecursiveMutex mtxSend;
RecursiveMutex mtxAddDelSess;
RecursiveMutex mtxCount;
RecursiveMutex mtxDNS;
    ...
    for (int i = 0; i < MaxClients; i++) {
        ...
        if (sessions[i].Socket != -1) {
            ...
            if ((sessions[i].EchoMask & Em) // Echo inet data if source mask bits match
                    ...
                    && (sessions[i].dead == false) // dead socket?
    ...
    while (!ShutDownServer) {
        ...
        if (abuff != NULL) {
            ...
            if (!((abuff->EchoMask) & (srcSTATS | srcSYSTEM)))
                dup  = dupFilter.check(abuff, 30);   // Check for duplicates within 30 second window
    ...
    if (rfbuf != NULL) {
        if (tncPresent ) {
            if ((abuff->allowdup == false)              // Prevent infinite loop!
                ...
                if ( (rc = pthread_create(&tid, NULL, ACKrepeaterThread, (void *) abuff) != 0)) {          // Make sure it starts
            ...
            msg << "Sending to TNC: " << rfbuf << endl; //debug only
            conQueue.write(cp, 0);
            ...
            if (State == BASE) {        // Internet to RF messaging handler
                ...
                if ((atemp.aprsType == APRSMSG) && (atemp.msgType == APRSMSGQUERY)) {
                    ...
                    if ((respondToIgateQueries) && (stricmp(szServerCall.c_str(), atemp.stsmDest.c_str()) == 0)
                        ...
                        cerr << "Ignored IGATE query from " << atemp.ax25Source << endl;
                    }
                    ...
                    if ((respondToAprsdQueries) && (stricmp(szServerCall.c_str(), atemp.stsmDest.c_str()) == 0)
                        ...
                        cerr << "Ignored APRSD query from " << atemp.ax25Source << endl;
                    }
                }
                ...
                if (atemp.aprsType == APRSERROR) {
                    //if(pthread_mutex_lock(pmtxCount) != 0)
                    //    cerr << "Unable to lock pmtxCount - TCPSessionThread - error counter." << endl;
                    countLock.get();
                    error_cnt++;
                    countLock.release();
                    //if(pthread_mutex_unlock(pmtxCount) != 0)
                    //    cerr << "Unable to unlock pmtxCount - TCPSessionThread - error counter." << endl;
                }
...
void *TCPServerThread(void *p)
{
    ...
    // port to time out.
    int reuse = 1;
    setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (char *)&reuse, (socklen_t)sizeof(reuse));
    ...
    if (bind(s, (SA *)&server, sizeof(server))) {
        close(s);
        char buf[100];
        throw BindException(strerror_r(errno, buf, sizeof(buf)));
    }

    //if (bind(s, (SA *)&server, sizeof(server)) <  0) {
    //    perror("TCPServerThread bind error");
    //    ShutDownServer = true;
    //    return NULL;
    //}

    cout << "TCP Server listening on port " << sp->ServerPort << endl;
    ...
    for(;;) {
        ...
        session->Socket = accept(s, (SA *)&client, &i);
...
#define UDPSIZE 256
    ...
    */
    ...
    for (;;) {                          // Loop forever
        ...
        i = recvfrom(s, buf, UDPSIZE, 0, (SA *) &client, &client_address_size) ; //Get client udp data
...
void *TCPConnectThread(void *p)
{
    ...
    do {
        ...
        if (gethostbyname_r(pcp->RemoteName, &hostinfo_d, h_buf, sizeof(h_buf), &hostinfo, &h_err) == -1)
            hostinfo = NULL;

        if (!hostinfo)
        {
            throw UnknownHostException(hstrerror(h_err));
        }
        ...
        if (state == 1) {
            ...
            if (( rc = connect(clientSocket,(SA *)&host, length)) < 0) {
                close(clientSocket);
                ostringstream os;
        ...
        if (state == 2) {
            ...
            setsockopt(clientSocket, SOL_SOCKET, SO_KEEPALIVE, (char *)&optval, (socklen_t)sizeof(optval));
...
int SendFiletoClient(int session, char *szName)
{
    ...
    do {
        ...
        if (strlen(Line) > 0) {
            ...
            //if (rc == -1) {
            if ((size_t)send(session, Line, nbytes, 0) != nbytes) {
            //if (rc < 0) {
...
int main(int argc, char *argv[])
{
    ...
    //#endif
        ...
        strcpy(szAPRSDPATH,">");
        strncat(szAPRSDPATH, APRSDTOCALL, 64);
