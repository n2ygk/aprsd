...
//extern vector<SessionParams> sessions;
...
extern std::string szServerCall;    // This servers "AX25 Source Call" (user defined)
extern std::string szJAVAMSG;
extern std::string szAPRSDPATH;     // Servers "ax25 dest address + path" eg: >APD213,TCPIP*:
extern std::string passDefault;     // Default passcode is "-1"
...
extern std::vector<std::string> posit_rfcall;
//extern string* stsmDest_rfcall[];
extern std::vector<std::string> stsmDest_rfcall;
//extern string* rfcall[];   //list of stations gated to RF full time (all packets)
extern std::vector<std::string> rfcall;
...
bool AddSessionInfo(int s, const string& userCall, const string& szPeer, int port, const string& pgmVers);
void CloseAllSessions(void);
void BroadcastString(const char *cp);
//int SendFiletoClient(int session, const char* szName);
int SendFiletoClient(int, const string&);
